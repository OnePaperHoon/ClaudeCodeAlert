...
6. Claude Code 슬래시 커맨드 `/cca-off`, `/cca-on`을 `~/.claude/commands/`에 설치합니다.
...
| `cca status`    | 현재 설치 상태 확인 (어느 이벤트가 활성? 사운드/메시지는?) |
...
`cca init`은 Claude Code 슬래시 커맨드도 함께 설치합니다. CLI를 열지 않고 세션 안에서 바로 음소거할 수 있습니다.

| 커맨드 | 동작 |
|--------|------|
| `/cca-off` | 알림 음소거 (`~/.claude/notifications.disabled` 생성) |
| `/cca-on`  | 음소거 해제 |

`cca disable`/`enable`(CLI)과 **동일한 플래그**를 토글하므로 상태가 항상 일치합니다. 설치 위치는 `~/.claude/commands/cca-off.md`, `cca-on.md`이며 `cca uninstall`에서 함께 제거할 수 있습니다.
...
`command`가 우리 dispatcher를 가리키는 hook만 골라서 제거합니다. 다른 hook은 그대로 남습니다. dispatcher 스크립트, 설정 파일, 슬래시 커맨드(`/cca-off`, `/cca-on`), 사용자가 등록한 사운드 파일도 함께 지울지 선택할 수 있습니다.
...
| `/cca-off`·`/cca-on`이 안 보임 | Claude Code 새 세션에서 다시 시도하세요. `~/.claude/commands/`에 파일이 있는지도 확인. |

## 참고: CodexAlert

OpenAI Codex CLI도 쓴다면 같은 아키텍처의 자매 라이브러리 [`codex-alert`](https://www.npmjs.com/package/codex-alert) (`cda`)를 함께 설치할 수 있습니다. Codex 안에서는 `cda-off`/`cda-on` 스킬로 토글합니다. 음소거 플래그가 `~/.claude`와 `~/.codex`로 분리되어 있어 한 시스템에서 서로 독립적으로 동작합니다.

## 변경 이력

### 1.0.0
- 정식 1.0 릴리스.
- Claude Code 슬래시 커맨드 `/cca-off`, `/cca-on` 추가 — CLI 없이 세션 안에서 알림 음소거 토글. `cca init`이 설치하고 `cca uninstall`에서 제거.
- `cca status`에 토글 커맨드 설치 여부 표시.
- `cca disable`/`enable`(CLI)과 동일한 음소거 플래그를 공유 → 상태 정합 보장.
